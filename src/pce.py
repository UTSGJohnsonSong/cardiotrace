...
def decision_curve(risks: dict[str, pd.Series], d: pd.DataFrame, horizon: float,
    ...
    if not np.isfinite(d[["followup_years", "cvd_death", "competing_death", "wtmec2yr"]]).all().all():
        raise ValueError("Missing outcome/design input")
    if (d.wtmec2yr <= 0).any():
        raise ValueError("Survey weights must be positive")
